...
const Administration = (props) => {
    ...
    const administration = locations.filter(location => location.type === "administration");
    const administrationNames = administration.map(item => item.name);

    // declarinTotleComponent
    const overviewItems = [];

    for (let i = 0; i < administrationNames.length; i++) {
        overviewItems.push({title: administrationNames[i], count: 0});
    }
        ...
        <>
            ...
            />
                <div className="inventory-table">
                    ...
                    {administrationNames.length === 0 ? <div style={{padding:'20px'}}>No locations avaliable in this type</div> : <div className="overview-grid" style={{padding:'20px'}}>
