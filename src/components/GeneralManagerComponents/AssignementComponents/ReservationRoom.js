...
const ReservationRoom = (props) => {
    ...
    const reservationRoomNames = reservationRoom.map(item => item.name);

    // declarinTotleComponent
    const overviewItems = [];

    for (let i = 0; i < reservationRoomNames.length; i++) {
        overviewItems.push({title: reservationRoomNames[i], count: 0});
    }
        ...
        <>
            ...
            />
                <div className="inventory-table">
                    ...
                    {reservationRoomNames.length === 0 ?
                        <div style={{padding: '20px'}}>No locations avaliable in this type</div> :
