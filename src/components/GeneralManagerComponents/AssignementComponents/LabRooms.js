...
const LabRooms = (props) => {
    const outlet = useOutlet();
    const [locations, setLocations] = useState([]);
    const labRooms = locations.filter(location => location.type === "lab_rooms");
    const labRoomsNames = labRooms.map(item => item.name);

    // declarinTotleComponent
    const overviewItems = [];

    for (let i = 0; i < labRoomsNames.length; i++) {
        overviewItems.push({title: labRoomsNames[i], count: 0});
    }
        ...
        <>
            ...
            />
                <div className="inventory-table">
                    ...
                    {labRoomsNames.length === 0 ? <div style={{padding:'20px'}}>No locations avaliable in this type</div> : <div className="overview-grid" style={{padding:'20px'}}>
