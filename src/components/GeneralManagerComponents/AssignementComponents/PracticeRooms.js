...
const PracticeRooms = (props) => {
    ...
    const practiceRooms = locations.filter(location => location.type === "practice_rooms");
    const practiceRoomsNames = practiceRooms.map(item => item.name);

    // declarinTotleComponent
    const overviewItems = [];

    for (let i = 0; i < practiceRoomsNames.length; i++) {
        overviewItems.push({title: practiceRoomsNames[i], count: 0});
    }

    const OverviewItem = ({title, count, index}) => {

        return (
            <div key={index} className="overview-item" onClick={() => {
                console.log(index)
            }}>
        ...
        <>
            ...
            />
                <div className="inventory-table">
                    ...
                    {practiceRoomsNames.length === 0 ? <div style={{padding:'20px'}}>No locations avaliable in this type</div> : <div className="overview-grid" style={{padding:'20px'}}>
