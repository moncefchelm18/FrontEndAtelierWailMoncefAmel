...
const Allocations = () => {
    ...
    const handleActiveAction = (allocation) => {
        return (
            <>
                <button onClick={() => handlDeleteAllocation(allocation)}>
    ...
    const handlDeleteAllocation = (allocation) => {
        ...
        axios.delete(`http://172.20.10.4:8000/allocate/${allocation.id}`, config)
            .then((response) => {
                ...
                pendingAllocationData(prevData => prevData.filter(item => item.id !== allocation.id));
            })
            .catch((error) => {
            ...
            {showPendingAllocationsTable &&
                <>
                    <PendingAllocationsTable
                        ...
                        currentPage={currentPage}
                        handleActiveAction={(allocation) => handleActiveAction(allocation)}
                        searchValueState={searchValueState}
                    />
                </>
            }
