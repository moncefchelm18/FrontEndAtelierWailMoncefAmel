...
const LocationsAdmin = () => {
    ...
    const getUpdatedData = (data) =>{
        ...
        setLocations([...locations, data['the new location']])
    }

    const handleAction = (location) => {
        return(
            <>
                <button className="action-button" onClick={() => handleUpdate(location)} >
                ...
                <button onClick={() => handleDelete(location.id)} className="action-button">
    ...
    // updatingForm
    const LocationUpdatingForm = (props) => {
        const [name, setName] = useState(props.location.name);
        const [type, setType] = useState(props.location.type)
        const [description, setDescription] = useState(props.location.discription);
            ...
            fetch(`http://127.0.0.1:8000/location/${props.location.id}/`, {
                ...
                .then(data => {
                    ...
                    setLocations(locations.map(location => {
                        if (location.id === props.location.id) {
                            return data.data;
                            console.log(data);
                        }
                        return location;
                    }));
                    setUpdateMessage(<p style={{ color: "green", paddingLeft: "20px", paddingTop: "10px" }}>Location updated successfully!</p>);
